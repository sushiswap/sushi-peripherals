...
/// @title Sushi V3 Staker Interface
/// @notice Allows staking nonfungible liquidity tokens in exchange for reward tokens
interface ISushiV3Staker is IERC721Receiver, IMulticall {
  /// @param rewardToken The token being distributed as a reward
  /// @param pool The Sushi V3 pool
  ...
  /// @notice Withdraws a Sushi V3 LP token `tokenId` from this contract to the recipient `to`
  /// @param tokenId The unique identifier of an Sushi V3 LP token
  /// @param to The address where the LP token will be sent
  /// @param data An optional data array that will be passed along to the `to` address via the NFT safeTransferFrom
  function withdrawToken(uint256 tokenId, address to, bytes memory data) external;

  /// @notice Stakes a Sushi V3 LP token
  /// @param key The key of the incentive for which to stake the NFT
  /// @param tokenId The ID of the token to stake
  function stakeToken(IncentiveKey memory key, uint256 tokenId) external;

  /// @notice Unstakes a Sushi V3 LP token
  ...
  function getRewardInfo(
    ...
    uint256 tokenId
  ) external returns (uint256 reward, uint256 maxReward, uint160 secondsInsideX128);
  ...
  /// @notice Event emitted when a Sushi V3 LP token has been staked
  /// @param tokenId The unique identifier of an Sushi V3 LP token
  /// @param liquidity The amount of liquidity staked
  /// @param incentiveId The incentive in which the token is staking
  event TokenStaked(uint256 indexed tokenId, bytes32 indexed incentiveId, uint128 liquidity);

  /// @notice Event emitted when a Sushi V3 LP token has been unstaked
  /// @param tokenId The unique identifier of an Sushi V3 LP token
  /// @param incentiveId The incentive in which the token is staking
