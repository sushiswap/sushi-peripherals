...
library RewardMath {
  ...
  function computeRewardAmount(
    ComputeRewardAmountParams memory params
  ) internal pure returns (uint256 reward, uint256 maxReward, uint160 secondsInsideX128) {
    // this should never be called before the start time
    assert(params.currentTime >= params.startTime);

    // this operation is safe, as the difference cannot be greater than 1/stake.liquidity
    secondsInsideX128 =
      (params.secondsPerLiquidityInsideX128 - params.secondsPerLiquidityInsideInitialX128) *
      params.liquidity;

    uint256 totalSecondsUnclaimedX128 = ((Math.max(params.endTime, params.currentTime) - params.startTime) << 128) -
